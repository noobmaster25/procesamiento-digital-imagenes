...
public class ImageFFT {
    ...
    /**
     * Creates an ImageFFT for the specified image, applying the specified
    ...
    /**
     * Transforms data via a forward or inverse FFT, as appropriate. An inverse
     * transform is computed if the previous transform was in the forward
     * direction; otherwise, the forward transform is computed.
     */
     ...
     * @exception FFTException if the data are in spectral form; an image can be
     * created only from data in the spatial domain.
     */
    ...
    /**
     * Returns the amplitude spectrum of an image, as another image. The data
     * are shifted such that the DC component is at the image centre, and scaled
     * logarithmically so that low-amplitude detail is visible.
     *
     * @return shifted spectrum, as an image.
     * @exception FFTException if spectral data are not available (e.g. because
     * last transform was in the inverse direction).
     */
    ...
    /**
     * Returns the amplitude spectrum of an image, as another image. The data
     * are unshifted and are scaled logarithmically so that low-amplitude detail
     * is visible.
     *
     ...
     * @return magnitude at the specified point, or zero if point does not
     * exist.
     ...
     *
     * @param radius filter radius
     * @exception FFTException if spectral data are not available or filter
     * radius is invalid.
     */
    public double transpolarRapio(String radio) {
        double nuevoRadio = 0;
        String validarNumero = "^[0-9]+$";
      
        if (radio.matches(validarNumero)) {
            int radioEntero = Integer.valueOf(radio);

            if (radioEntero >= 0 && radioEntero <= 360) {
                nuevoRadio = (radioEntero * 1) / 360.0;
                return nuevoRadio;
            }
        }
        JOptionPane.showMessageDialog(null,"el valor introducido no es numero");
    ...
    ///////////////////////////// PRIVATE METHODS ////////////////////////////
    /**
     * Computes the power of two for which the corresponding value equals or
     * exceeds the specified integer.
     *
    ...
    /**
     * Reorders an array of data to prepare for an FFT. The element at index i
     * is swapped with the element at an index given by the bit-reversed value
     * of i.
     *
    ...
    /**
     * Collects magnitudes from spectral data, storing them in the specified
     * array.
     *
